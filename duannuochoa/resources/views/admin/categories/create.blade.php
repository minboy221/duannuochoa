...
<main class="ml-64 min-h-screen p-8">
    <div class="mb-8">
        <h2 class="text-3xl font-extrabold text-on-background tracking-tight">Thêm Danh mục Mới</h2>
    </div>
    ...
    <div class="bg-surface-container-lowest p-6 rounded-xl shadow-sm border border-surface-container max-w-2xl">
        <form action="{{ route('admin.categories.store') }}" method="POST" novalidate>
            ...
            <div class="mb-6">
                ...
                <textarea name="description" class="w-full rounded-lg border-gray-300 p-3" rows="4"></textarea>
                @error('description') <span class="text-error text-sm">{{ $message }}</span> @enderror
            </div>

            <div class="flex gap-4">
                <button type="submit" class="bg-primary text-white px-6 py-2 rounded-lg font-bold">Lưu Danh mục</button>
