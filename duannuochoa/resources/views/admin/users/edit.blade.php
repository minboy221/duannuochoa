    ...
    <div class="bg-surface-container-lowest p-6 rounded-xl shadow-sm border border-surface-container max-w-2xl">
        <form action="{{ route('admin.users.update', $user) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="username" value="{{ $user->username }}" class="w-full rounded-lg border-gray-300 p-3" required>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-bold mb-2">Họ & Tên</label>
                <input type="text" name="full_name" value="{{ $user->full_name }}" class="w-full rounded-lg border-gray-300 p-3" required>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-bold mb-2">Email</label>
                <input type="email" name="email" value="{{ $user->email }}" class="w-full rounded-lg border-gray-300 p-3" required>
            </div>
            <div class="mb-4">
